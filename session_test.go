...
func newManager(t *testing.T, options ghostline.Options) *ghostline.Manager {
	t.Helper()
	if options.OutputDir == "" {
		options.OutputDir = t.TempDir()
	}
	manager, err := ghostline.New(options)
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	t.Cleanup(func() {
...
func TestManagerStartConfiguresSizeAndEnvironment(t *testing.T) {
	manager := newManager(t, ghostline.Options{
		DefaultSize: ghostline.Size{Columns: 90, Rows: 28},
	})
	session, err := manager.Start(context.Background(), ghostline.SessionOptions{
		Name:        "configured",
		Directory:   t.TempDir(),
		Command:     "sh",
...
func TestSessionHandleCannotAffectReplacement(t *testing.T) {
	manager := newManager(t, ghostline.Options{})
	oldSession, err := manager.Start(context.Background(), ghostline.SessionOptions{
		Name:      "reused",
		Directory: t.TempDir(),
		Command:   "sleep 30",
	})
	...
	newSession, err := manager.Start(context.Background(), ghostline.SessionOptions{
		Name:      "reused",
		Directory: t.TempDir(),
		Command:   "sleep 30",
	})
...
func TestManagerErrorsAreInspectable(t *testing.T) {
	...
	if _, err := manager.Start(context.Background(), options); !errors.Is(err, ghostline.ErrSessionExists) {
		t.Fatalf("duplicate error = %v", err)
	}
	if _, err := manager.Start(context.Background(), ghostline.SessionOptions{Name: "../unsafe"}); !errors.Is(err, ghostline.ErrInvalidSessionName) {
		t.Fatalf("invalid name error = %v", err)
	}
	if err := manager.Close(); err != nil {
		t.Fatalf("Close: %v", err)
	}
	if _, err := manager.Start(context.Background(), ghostline.SessionOptions{Name: "closed"}); !errors.Is(err, ghostline.ErrClosed) {
